...
def gen_piano_track(track, beat, note_last, chord, mode, vel, first_beat, third_beat, onset, sm, grace_flag, cadence, last, key):
    ...
    if prob > onset:
        ...
        if ((grace > grace_prob_1 and first_beat) or (grace > grace_prob_2 and third_beat)) and (done == 0):
            ...
            #chord
            if first_beat:
                on = midi.NoteOnEvent(tick = 0, velocity = 50, pitch = chord[0])
                track.append(on)
                on = midi.NoteOnEvent(tick = 0, velocity = 40, pitch = chord[1])
                track.append(on)
                on = midi.NoteOnEvent(tick = 0, velocity = 40, pitch = chord[2])
                track.append(on)
                on = midi.NoteOnEvent(tick = 0, velocity = 40, pitch = chord[3])
                track.append(on)
        ...
        if grace_flag == 1:
            ...
            if first_beat:
                on = midi.NoteOnEvent(tick = 0, velocity = 50, pitch = chord[0])
                track.append(on)
                on = midi.NoteOnEvent(tick = 0, velocity = 50, pitch = chord[0]+24)
                track.append(on)
                on = midi.NoteOnEvent(tick = 0, velocity = 40, pitch = chord[1])
                track.append(on)
                on = midi.NoteOnEvent(tick = 0, velocity = 40, pitch = chord[2])
                ...
                off = midi.NoteOffEvent(tick = beat*2, velocity = 50, pitch = chord[0])
                track.append(off)
                off = midi.NoteOffEvent(tick = 0, velocity = 50, pitch = chord[0]+24)
                track.append(off)
