...
def inception_phase_20_1(sm, num, key):
    ...
    drum_en = [0] * len(scale_a) + [1] * len(scale_b) + [1] * len(scale_c)
    ...
    for chord_cnt in range(len(chord)):
        ...
        for measure_cnt in range(measure_per_chord):
            ...
            first_beat = 0
            third_beat = 0
            #gen_bass_track_full.gen_bass_track_full(bass_track, beat, chord[chord_cnt], velocity_bass, key)
            #gen_string_track_full.gen_string_track_full(string_track, beat, chord[chord_cnt], velocity_string, string_en[chord_cnt], last, key)
